...
def main():
    ...
    action_config_file = ""
    script_dir = Path(__file__).parent.absolute()
    # 脚本目录通常是 .../joint_breakin_ros/src/leg_breakin/src/leg_breakin_roban2_v17
    # 需要向上4级找到工作空间根目录
    workspace_root = script_dir
    for _ in range(5):  # 最多向上5级
        config_file_path = workspace_root / "config" / "leg_breakin" / "leg_breakin_roban2_v17_config.yaml"
    ...
    if not action_config_file:
        possible_paths = [
            script_dir / ".." / ".." / ".." / ".." / "config" / "leg_breakin" / "leg_breakin_roban2_v17_config.yaml",
            script_dir / ".." / ".." / ".." / ".." / ".." / "config" / "leg_breakin" / "leg_breakin_roban2_v17_config.yaml",
        ]
