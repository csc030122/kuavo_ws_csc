...
def signal_handler(signum, frame):
    """处理中断信号"""
    global stop_program
    print(f"\n{time.strftime('%H:%M:%S', time.localtime())} 收到停止信号，正在安全停止Kuavo5V55腿部磨线程序...")
    stop_program = True
    ...
    # 立即退出程序
    print(f"{time.strftime('%H:%M:%S', time.localtime())} Kuavo5V55腿部磨线程序正在退出...")
    sys.exit(0)
...
def main():
    ...
    action_config_file = ""
    script_dir = Path(__file__).parent.absolute()
    # 脚本目录通常是 .../joint_breakin_ros/src/leg_breakin/src/leg_breakin_kuavo5_v55
    # 需要向上4级找到工作空间根目录
    workspace_root = script_dir
    for _ in range(5):  # 最多向上5级
        config_file_path = workspace_root / "config" / "leg_breakin" / "leg_breakin_kuavo5_v55_config.yaml"
    ...
    if not action_config_file:
        possible_paths = [
            script_dir / ".." / ".." / ".." / ".." / "config" / "leg_breakin" / "leg_breakin_kuavo5_v55_config.yaml",
            script_dir / ".." / ".." / ".." / ".." / ".." / "config" / "leg_breakin" / "leg_breakin_kuavo5_v55_config.yaml",
        ]
    ...
    success = False
    try:
        # 调用函数：Kuavo5V53腿部磨线运动（ec_master 绑定名仍为 Kuavo5V53LegBreakin，与 v53 共用实现）
        ...
        if not success:
            print("\033[1;31m✘ Kuavo5V55磨线运动失败或被中断\033[0m")
        ...
        if success:
            print("\033[1;32m✓ Kuavo5V55磨线运动完成\033[0m")
