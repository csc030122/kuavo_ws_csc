/*
 * ============================================================================
 * 机型配置说明：kuavo_v55_dual
 * ============================================================================
 * 当前代码适配的机型：kuavo_v55_dual（双CAN版本，机器人版本55）
 * 
 * 电机配置：
 *   - 总电机数量：16个（ID 1-16）
 *   - 手臂电机：14个（ID 1-14，左臂1-7，右臂8-14）
 *   - 头部电机：2个（ID 15-16）
 * 
 * 动作序列配置：
 ...
 *     * ID 9 = -ID 2（反方向，原ID 1）
 *     * ID 10 = -ID 3（反方向，原ID 2）
 *     * ID 11 = ID 4（同方向，原ID 3）
 *     * ID 12 = -ID 5（反方向，原ID 4）
 *     * ID 13 = -ID 6（反方向，原ID 5）
 *     * ID 14 = ID 7（同方向，原ID 6）
 *   - 头部动作：2关节，7帧动作序列
 * 
 * 注意：此配置仅适用于 kuavo_v55_dual 型机器人
 * ============================================================================
 */
...
public:
    ArmBreakinNode(int argc, char** argv) : nh_(nullptr), arm_ready_(false), arm_running_(false), start_received_(false), 
        ...
        ros::init(argc, argv, "arm_breakin_kuavo_v55_dual_node");
        ...
        if (action_config_file_path_.empty()) {
            ...
            if (len != -1) {
                ...
                if (pos1 != std::string::npos) {
                    ...
                    search_paths.push_back(join_path(workspace_root, {"config", "arm_breakin", "arm_breakin_kuavo_v55_dual_config.yaml"}));
                    ...
                    for (int i = 0; i < 10; ++i) {  // 最多向上10级
                        std::string test_path = join_path(current_path, {"config", "arm_breakin", "arm_breakin_kuavo_v55_dual_config.yaml"});
            ...
            if (ros_workspace != nullptr) {
                search_paths.push_back(join_path(ros_workspace, {"config", "arm_breakin", "arm_breakin_kuavo_v55_dual_config.yaml"}));
            }
            ...
            if (getcwd(cwd, sizeof(cwd)) != nullptr) {
                ...
                search_paths.push_back(join_path(workspace_path, {"config", "arm_breakin", "arm_breakin_kuavo_v55_dual_config.yaml"}));
                search_paths.push_back(join_path(workspace_path, {"..", "config", "arm_breakin", "arm_breakin_kuavo_v55_dual_config.yaml"}));
                // 如果在src目录下
        ...
        std::cout << get_timestamp() << " 手臂磨线ROS节点已启动 [机型: kuavo_v55_dual]" << std::endl;
    ...
    // 从YAML文件读取动作配置
    bool loadActionConfig(std::vector<std::array<double,7>>& left_arm_actions,
        ...
        try {
            ...
            if (action_node["left_arm_actions"]) {
                ...
                for (const auto& frame : action_node["left_arm_actions"]) {
                    std::array<double, 7> frame_data;
                    for (size_t i = 0; i < frame.size() && i < 7; ++i) {
    ...
    void run() {
        ...
        // ====================================================================
        // kuavo_v55_dual 机型配置：电机数量与ID范围
        // ====================================================================
        // 电机配置：16个电机（ID 1-16）
        //   - 手臂：14个电机（ID 1-14）
        //     * 左臂：ID 1-7（7个关节）
        //     * 右臂：ID 8-14（7个关节，通过左臂动作映射）
        //   - 头部：2个电机（ID 15-16）
        // ====================================================================
        auto motor_states = actuator_->get_motor_state();
        ...
        for (size_t idx = 0; idx < motor_states.size(); ++idx) {
            ...
            if (motor_id >= 0x1 && motor_id <= 0x10) {  // ID 1-16 (0x10 = 16) - kuavo_v55_dual机型
        ...
        if (target_indices.empty()) {
            std::cout << get_timestamp() << " [错误] 未找到目标电机（ID 1-16，kuavo_v55_dual机型）" << std::endl;
            publishArmReady(false);
            publishArmRunning(false);  // 错误时也发布False
            return;
        }
        
        std::cout << get_timestamp() << " [kuavo_v55_dual] 找到 " << target_indices.size() 
                  << " 个目标电机（手臂1-14 + 头部15-16）" << std::endl;
        ...
        // ====================================================================
        // kuavo_v55_dual 机型配置：动作序列定义（从YAML文件读取）
        // ====================================================================
        ...
        // ====================================================================
        // kuavo_v55_dual 机型配置：电机ID到动作序列的映射关系
        // ====================================================================
        // 映射规则（仅适用于 kuavo_v55_dual）：
        //   左臂（ID 1-7）：直接使用左臂动作序列的对应关节
        //     - ID 1 -> left_offsets[0] (左臂关节1，新增)
        //     - ID 2 -> left_offsets[1] (左臂关节2，原ID 1)
        //     - ID 3 -> left_offsets[2] (左臂关节3，原ID 2)
        //     - ID 4 -> left_offsets[3] (左臂关节4，原ID 3)
        //     - ID 5 -> left_offsets[4] (左臂关节5，原ID 4)
        //     - ID 6 -> left_offsets[5] (左臂关节6，原ID 5)
        //     - ID 7 -> left_offsets[6] (左臂关节7，原ID 6)
        //   
        //   右臂（ID 8-14）：通过左臂动作序列映射
        //     - ID 8  -> left_offsets[0]  (右臂关节1 = 左臂关节1，同方向，新增)
        //     - ID 9  -> -left_offsets[1] (右臂关节2 = -左臂关节2，反方向，原ID 7)
        //     - ID 10 -> -left_offsets[2] (右臂关节3 = -左臂关节3，反方向，原ID 8)
        //     - ID 11 -> left_offsets[3]  (右臂关节4 = 左臂关节4，同方向，原ID 9)
        //     - ID 12 -> -left_offsets[4] (右臂关节5 = -左臂关节5，反方向，原ID 10)
        //     - ID 13 -> -left_offsets[5] (右臂关节6 = -左臂关节6，反方向，原ID 11)
        //     - ID 14 -> left_offsets[6]  (右臂关节7 = 左臂关节7，同方向，原ID 12)
        //   
        //   头部（ID 15-16）：直接使用头部动作序列
        //     - ID 15 -> head_offsets[0] (头部关节1，原ID 13)
        //     - ID 16 -> head_offsets[1] (头部关节2，原ID 14)
        // ====================================================================
        auto send_offsets = [&](const std::array<double,7>& left_offsets, const std::array<double,2>& head_offsets = {0.0, 0.0}) {
            ...
            for (size_t i = 0; i < target_indices.size(); ++i) {
                ...
                if (motor_id >= 1 && motor_id <= 7) {
                    ...
                    // 右臂关节2 = -左臂关节2（反方向，原ID 7）
                    off = -left_offsets[1];
                } else if (motor_id == 10) {
                    // 右臂关节3 = -左臂关节3（反方向，原ID 8）
                    off = -left_offsets[2];
                } else if (motor_id == 11) {
                    // 右臂关节4 = 左臂关节4（同方向，原ID 9）
                    off = left_offsets[3];
                } else if (motor_id == 12) {
                    // 右臂关节5 = -左臂关节5（反方向，原ID 10）
                    off = -left_offsets[4];
                } else if (motor_id == 13) {
                    // 右臂关节6 = -左臂关节6（反方向，原ID 11）
                    off = -left_offsets[5];
                } else if (motor_id == 14) {
                    // 右臂关节7 = 左臂关节7（同方向，原ID 12）
                    off = left_offsets[6];
                } else if (motor_id == 15) {
                    // 头部关节1（原ID 13）
                    off = head_offsets[0];
        ...
        auto do_frame_pair = [&](const std::array<double,7>& start_left,
                                  const std::array<double,7>& end_left,
