/*
 * ============================================================================
 * 机型配置说明：kuavo_v56_dual
 * ============================================================================
 * 当前代码适配的机型：kuavo_v56_dual（双CAN版本，机器人版本56）
 * 
 * 电机配置：
 *   - 总电机数量：16个（ID 1-16）
 ...
 *   - 头部动作：2关节，7帧动作序列
 * 
 * 注意：此配置仅适用于 kuavo_v56_dual 型机器人
 * ============================================================================
 */
...
public:
    ArmBreakinNode(int argc, char** argv) : nh_(nullptr), arm_ready_(false), arm_running_(false), start_received_(false), 
        ...
        ros::init(argc, argv, "arm_breakin_kuavo_v56_dual_node");
        ...
        if (action_config_file_path_.empty()) {
            ...
            if (len != -1) {
                ...
                if (pos1 != std::string::npos) {
                    ...
                    search_paths.push_back(join_path(workspace_root, {"config", "arm_breakin", "arm_breakin_kuavo_v56_dual_config.yaml"}));
                    ...
                    for (int i = 0; i < 10; ++i) {  // 最多向上10级
                        std::string test_path = join_path(current_path, {"config", "arm_breakin", "arm_breakin_kuavo_v56_dual_config.yaml"});
            ...
            if (ros_workspace != nullptr) {
                search_paths.push_back(join_path(ros_workspace, {"config", "arm_breakin", "arm_breakin_kuavo_v56_dual_config.yaml"}));
            }
            ...
            if (getcwd(cwd, sizeof(cwd)) != nullptr) {
                ...
                search_paths.push_back(join_path(workspace_path, {"config", "arm_breakin", "arm_breakin_kuavo_v56_dual_config.yaml"}));
                search_paths.push_back(join_path(workspace_path, {"..", "config", "arm_breakin", "arm_breakin_kuavo_v56_dual_config.yaml"}));
                // 如果在src目录下
        ...
        std::cout << get_timestamp() << " 手臂磨线ROS节点已启动 [机型: kuavo_v56_dual]" << std::endl;
    ...
    void run() {
        ...
        for (size_t idx = 0; idx < motor_states.size(); ++idx) {
            ...
            if (motor_id >= 0x1 && motor_id <= 0x10) {  // ID 1-16 (0x10 = 16) - kuavo_v56_dual机型
        ...
        if (target_indices.empty()) {
            std::cout << get_timestamp() << " [错误] 未找到目标电机（ID 1-16，kuavo_v56_dual机型）" << std::endl;
            publishArmReady(false);
            publishArmRunning(false);  // 错误时也发布False
            return;
        }
        
        std::cout << get_timestamp() << " [kuavo_v56_dual] 找到 " << target_indices.size() 
        ...
        // ====================================================================
        // kuavo_v56_dual 机型配置：动作序列定义（从YAML文件读取）
        // ====================================================================
        ...
        // ====================================================================
        // kuavo_v56_dual 机型配置：电机ID到动作序列的映射关系
        // ====================================================================
        // 映射规则（仅适用于 kuavo_v56_dual）：
