...
`/humanoid_controller/real_initial_start` 服务用于触发机器人的初始化过程：机器人 cali 状态执行一次机器人缩腿，再执行一次机器人站立，用户需要在站立过程中用手扶住机器人以确保安全，实机才有该服务。

**VR 遥操说明（QuestControlFSM）**：在尚未订阅到 MPC `observation` 前，手柄侧用**长按右手 A 约 0.35 秒**触发本服务，等价于下位机终端按 `o`；短按易在 VR 刚连上时误触发，见 kuavodevlab#2995。收到观测后该长按逻辑不再占用 A 键上升沿。
