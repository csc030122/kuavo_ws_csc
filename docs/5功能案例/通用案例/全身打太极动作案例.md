...
4. H12遥控器执行全身打太极动作案例
 ...
 - 修改下位机路径为 `<kuavo-ros-opensource>/src/humanoid-control/h12pro_controller_node/config/customize_config.json` 配置文件。
 - 填入您想使用的遥控器按键组合，例如:
  ```bash
  "customize_action_LL_A": {
    "type": "shell",
    "command": "python3 /home/lab/kuavo-ros-opensource/src/demo/taiji/actions_player.py > /home/lab/taiji_output.txt 2>&1"
  },
  ```
 - 修改完后需要关机重启机器人。
