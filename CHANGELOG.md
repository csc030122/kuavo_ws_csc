...
- 更新强化学习运动控制框架文档，[文档链接](./docs/5功能案例/科研框架/强化学习运动控制框架合集.md)
...
## 新增功能
- 新增5代人型胸口LED灯光串口控制模块及ROS接口
- 新增主动手跟踪模式下手臂复位时头部自动回正功能，支持pico版本
- 新增轮臂VR增量遥操作功能，支持增量位置/姿态控制、底盘控制、腰部跟随等
- VR增量遥操增加54版本支持
- Kuavo Humanoid SDK 增加控制器相关接口（获取/切换/状态），KMPC运动学接口增加MPC模式判断保护
- 优化日志管理，自动清理冗余 coredump 并限制日志保留时长
...
## 修复问题
- 修复 VR 遥操相关问题：增量模式手臂复位未退出极速模式、5W 绝对模式手臂跟踪异常
- 修复步态控制问题：单步控制时序及摇杆导致踏步停不下来
- 修复轮臂启动问题（实物启动及 VR 轮臂参数错误）
- 修复轮臂底盘控制问题（键盘控制及嘉腾底盘独立控制）
- 修复 ruiwo_kp/kd 精度截断问题
- 统一手臂贝塞尔节点启动入口，避免与 websocket 节点冲突
- 修复 5 代限位标零时忽略腰部电机的问题
- 修复获取控制器列表顺序未按添加顺序的问题
- 修复 5 代机器人网络拓扑 DHCP CPU 占用问题
