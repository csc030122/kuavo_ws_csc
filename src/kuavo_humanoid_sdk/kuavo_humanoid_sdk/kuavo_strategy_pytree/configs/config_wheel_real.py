...
class config:
    class common:
        ...
        box_weight_kg = 15.25  # 箱子重量，单位千克

        contact_force_weight_transition_time = 0.3
        contact_force_interpolation_speed = 200.0
    ...
    class fake_tags:
        ...
        enable = True  # True 时跳过视觉识别
        pick_pos = (0.24, -1.49, 0.83)   # pick tag 虚假位置 (x,y,z) 米，ODOM 系
        pick_euler = (90, 0, 180)     # pick tag 虚假姿态 (roll,pitch,yaw) 度
        place_pos = (0.42, 1.52, 0.92)   # place tag 虚假位置
    ...
    class place:
        ...
        stand_in_tag_pos = (0.0, 0.0, 0.85)  # 放置位置站立位置在目标标签中的位置，单位米
        stand_in_tag_euler = (-np.deg2rad(90), np.deg2rad(90), 0.0)  # 放置位置站立位置在目标标签中的姿态，单位欧拉角（弧度）
        box_behind_tag = -0.2  # 箱子在tag后面的距离，单位米
        box_beneath_tag = 0.25  # 箱子在tag下方的距离，单位米
        box_left_tag = 0.0  # 箱子在tag左侧的距离，单位米
