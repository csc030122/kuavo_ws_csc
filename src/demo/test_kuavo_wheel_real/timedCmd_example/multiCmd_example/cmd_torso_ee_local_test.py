...
def execute_combined_torso_arm_tests():
    """组合发布躯干和双臂末端位姿数据，使用多指令同时执行"""
    global initialTorsoPose_
    
    # 初始化节点
    rospy.init_node('torso_arm_combined_publisher', anonymous=True)
    ...
    # 创建解析器
    parser = argparse.ArgumentParser(description='躯干和手臂组合测试程序')
    ...
    rospy.loginfo("开始发布躯干+手臂组合测试数据...")

    for idx, (name, desire_time, torso_rel_pose, left_arm_pose, right_arm_pose) in enumerate(test_cases, 1):
        # 解包躯干位置数据
...
def main():
    try:
        # 运行组合测试
        execute_combined_torso_arm_tests()
