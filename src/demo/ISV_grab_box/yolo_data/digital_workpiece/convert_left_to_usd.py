#!/usr/bin/env python3
"""Convert left.stp to a monolithic USD with an Isaac Sim matte PP material."""
...
SOURCE = Path(__file__).with_name("left.stp").resolve()
OUTPUT = Path(__file__).with_name("left.usd").resolve()
PART_NUMBER = "Land Rover LR028551"
BASE_COLOR_SRGB_8BIT = (24, 24, 24)
...
def create_material(stage):
    ...
    material = UsdShade.Material.Define(stage, "/Looks/BlackTexturedPPBumperFinisher")
    material_prim = material.GetPrim()
    material_prim.SetCustomDataByKey("description", "Deep-black non-metallic textured matte polypropylene exterior trim")
    material_prim.SetCustomDataByKey("partNumber", PART_NUMBER)
    ...
    mdl_shader = UsdShade.Shader.Define(stage, "/Looks/BlackTexturedPPBumperFinisher/OmniPBR")
    ...
    mdl_shader.CreateInput("reflection_roughness_constant", Sdf.ValueTypeNames.Float).Set(0.78)
    ...
    preview_shader = UsdShade.Shader.Define(stage, "/Looks/BlackTexturedPPBumperFinisher/UsdPreviewSurface")
    ...
    preview_shader.CreateInput("roughness", Sdf.ValueTypeNames.Float).Set(0.78)
    preview_shader.CreateInput("opacity", Sdf.ValueTypeNames.Float).Set(1.0)
    preview_shader.CreateInput("ior", Sdf.ValueTypeNames.Float).Set(1.49)
