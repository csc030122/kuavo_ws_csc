...
class ActionPlayer:
    ...
    def generate_foot_trajectory_with_csv(self):
        ...
        for i, step in enumerate(self.step_control):
            ...
            if mode == 0:  # 双脚支撑 (SS)
                ...
                msg.swingHeightTrajectory.append(0.05)  # 摆动高度
                ...
                # 第二步：左脚摆动 (FS)
                msg.timeTrajectory.append(time + 0.6)
                msg.footIndexTrajectory.append(0)
                msg.swingHeightTrajectory.append(0.05)
                ...
                # 第三步：右脚摆动 (SF)
                msg.timeTrajectory.append(time + 1.2)
    ...
    def project_torso_to_line(self, torso_pos, foot_pos1, foot_pos2):
        """将躯干位置映射到脚部位置的连线上"""
        # 计算向量
        ...
        projected_pos = np.array(foot_pos1) + t * line_vec
        
        return projected_pos.tolist()
    ...
    def generate_arm_trajectory_with_csv(self, start_time):
        ...
        for step in self.step_control:
            ...
            # 每条手臂有7个维度，总共有14个维度
            arm_row_rad[5], arm_row_rad[6] = arm_row_rad[6], arm_row_rad[5]  # 交换第一条手臂的最后两个关节
            arm_row_rad[12], arm_row_rad[13] = arm_row_rad[13], arm_row_rad[12]  # 交换第二条手臂的最后两个关节
            # 添加手臂数据（弧度单位）
    ...
    def execute_action_with_csv(self):
        """执行动作序列"""
        if not self.step_control:
            rospy.logerr("未加载手臂数据")
            return
...
def main():
    ...
    parser.add_argument('csv_file', type=str, help='CSV文件路径', nargs='?', 
                       default=os.path.join(os.path.dirname(os.path.abspath(__file__)), "actions", "taiji_step_postprocess.csv"))
    ...
    try:
        # 如果指定了时间偏移
        if args.time_offset is not None and args.time_offset > 0:
            # 等待接收到MPC时间
            timeout = rospy.Duration(5.0)  # 5秒超时
            start_wait = rospy.Time.now()
            while not player.mpc_time_received:
                if (rospy.Time.now() - start_wait) > timeout:
                    rospy.logerr("等待MPC时间超时")
                    return
                rospy.sleep(0.1)
            ...
            # 设置调度参数
            target_time = player.mpc_time + args.time_offset
        ...
        # 执行动作序列
        player.execute_action_with_csv()
