...
namespace humanoid_controller
{
  ...
  DepthWalkController::DepthWalkController(const std::string& name,
                                       const std::string& config_file,
                                       ros::NodeHandle& nh,
                                       TopicLogger* ros_logger)
    : RLControllerBase(name, RLControllerType::DEPTH_LOCO_CONTROLLER, config_file, nh, ros_logger)
  {
  ...
  bool DepthWalkController::initialize()
  {
    ...
    ROS_INFO("[%s] DepthWalkController initialized", name_.c_str());
    return true;
  }

  void DepthWalkController::inferenceThreadFunc()
  {
    ros::Rate inference_rate(inference_frequency_);
    int pre_load_frames_num = 5;
  ...
  bool DepthWalkController::loadConfig(const std::string& rlParamFile)
  {
    bool verbose = false;
    ...
    loadData::loadCppDataType(rlParamFile, "leg_bias", leg_bias);
    loadData::loadCppDataType(rlParamFile, "stance_ratio", stance_ratio);
  ...
  void DepthWalkController::pause()
  {
  ...
  void DepthWalkController::resume()
  {
                                            ...
                                            const SensorData& sensor_data)
  {
    ...
    int depth_size = 8 * 36 * 64;
    Eigen::VectorXd depth_image = Eigen::VectorXd::Zero(depth_size);

    for(int i=0; i<depth_size; i++) {
      depth_image(i) = std::clamp(depth_[i], 0.0, 1.0);
    }
  ...
  bool DepthWalkController::inference(const Eigen::VectorXd& observation,
                                    Eigen::VectorXd& action)
  {
    try
    {
      ...
      size_t expected_input_length = expected_input_shape[0];

      if (expected_input_shape.size() == 2 )
      {
        expected_input_length=expected_input_shape[1];
      }
  ...
  Eigen::VectorXd DepthWalkController::updateRLcmd(const Eigen::VectorXd& measuredRbdState)
  {
    ...
    if (is_real_)
    {
      for (int i = 0; i < jointNum_ + jointArmNum_ + waistNum_; i++)
      {
        ...
        else if (JointControlModeRL_(i) == 2)
        {
          ...
          //double excess = torque[i] - std::clamp(torque[i], -torqueLimitsRL_[i], torqueLimitsRL_[i]);
          //cmd[i] -= excess;
  ...
  void DepthWalkController::actionToJointCmd(const Eigen::VectorXd& actuation,
  ...
  bool DepthWalkController::updateImpl(const ros::Time& time,
                                     const SensorData& sensor_data,
                                     const Eigen::VectorXd& measuredRbdState,
                                     kuavo_msgs::jointCmd& joint_cmd)
  {
  ...
  void DepthWalkController::preprocessSensorData(SensorData& sensor_data)
  {
  ...
  void DepthWalkController::initArmControl(const std::string& urdf_path)
  {
    ...
    if (arm_command_replacement_enabled_ && jointArmNum_ > 0)
    {
      try
      {
        ...
        // 注意：DepthWalkController 中 jointNum_ 是腿部关节数，waistNum_ 是腰部关节数，jointArmNum_ 是手臂关节数
  ...
  bool DepthWalkController::updateArmCommand(const ros::Time& time,
                                           const SensorData& sensor_data,
                                           kuavo_msgs::jointCmd& joint_cmd)
  {
    ...
    void DepthWalkController::initWaistControl()
  {
  ...
  void DepthWalkController::changeRuiwoMotorParamAsync(const std::string& param_name)
  {
    // 仿真环境下直接返回
    if (!is_real_)
    {
    ...
    std::thread([this, param_name]()
    {
      try
      {
        ...
        if (srv_change_motor_param_.call(srv))
        {
          ...
          {
            ROS_INFO("[DepthWalkController] Successfully changed Ruiwo motor param to: %s", param_name.c_str());
          }
          else
          {
            ROS_WARN("[DepthWalkController] Failed to change Ruiwo motor param: %s", srv.response.message.c_str());
          }
        }
        else
        {
          ROS_WARN("[DepthWalkController] Failed to call Ruiwo motor param service");
        }
      }
      catch (const ros::Exception& e)
      {
        ROS_ERROR("[DepthWalkController] ROS exception in changeRuiwoMotorParamAsync: %s", e.what());
      }
      catch (const std::exception& e)
      {
        ROS_ERROR("[DepthWalkController] Exception in changeRuiwoMotorParamAsync: %s", e.what());
      }
    }).detach();
  }

  // 更新腰部指令（可选功能，用于替换jointCmdMsg中的腰部部分）
  bool DepthWalkController::updateWaistCommand(const ros::Time& time,
                                             const SensorData& sensor_data,
                                             kuavo_msgs::jointCmd& joint_cmd)
  {
  ...
  void DepthWalkController::updateVelocityLimitsParam(ros::NodeHandle& nh)
  {
