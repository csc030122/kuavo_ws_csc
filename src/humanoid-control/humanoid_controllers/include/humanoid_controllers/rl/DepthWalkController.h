...
namespace humanoid_controller
{
  ...
  public:
     ...
     * 当估计的 roll/pitch 绝对值超过约 60°（判为跌倒）时返回 true，见实现与 AmpWalk 相同逻辑。
     * @return 判为跌倒时为 true
     */
     ...
     * 当步态接收器当前指令为 stance 站立（cmdStance_ ≥ 0.5）时返回 true，逻辑与 AmpWalk 一致；无接收器时默认 true。
     * @return 允许在站立指令下切出时为 true
     */
