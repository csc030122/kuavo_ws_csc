...
def get_user_input():
    ...
    print("请选择要执行的功能:")
    print("┌─" + "─"*56 + "─┐")
    print("│ 1. 上下斜坡 (up and downslope) - 6D轨迹控制                │")
    print("│ 2. 退出                                             │")
    print("└─" + "─"*56 + "─┘")
...
class EnhancedSlopeController:
    ...
    def simple_up_down_stairs(self, distance_x, stairs_angle, step_num, step_length):  # 输入x方向距离，坡度，除第一步和最后一步外，机器人需要进行的步数，机器人脚掌长度
        ...
        for step in range(step_num+1):  # 生成第一步和上斜坡的
            ...
            # 所有摆动相中间都加入支撑相
            step_swing_time = step_swing_time + contact_time
            time_traj.append(step_swing_time)
            foot_idx_traj.append(2)  # 接触相标志
            # 获取foot_poses_6d数组中最后一个数据的x和z值
            last_foot_pose = foot_poses_6d[-1]
            foot_x_current = last_foot_pose[0]  # 获取x坐标
            foot_z_current = last_foot_pose[2]  # 获取z坐标
            foot_poses_6d.append([
                foot_x_current, 0.0, foot_z_current, 0.0, 0.0, 0.0
            ])

            # 获取torso_poses_6d数组中最后一个数据的x和z值
            last_torso_pose = torso_poses_6d[-1]
            torso_x_current = last_torso_pose[0]  # 获取x坐标
            torso_z_current = last_torso_pose[2]  # 获取z坐标
            torso_poses_6d.append([
                torso_x_current, 0.0, torso_z_current, 0.0, 0.0, 0.0
            ])
