...
class WheelIncrementalPoseResult {
 private:
  ...
  // ##############################################################position##############################################################
  // ##############################################################position##############################################################
  // ---------------------------- chest position incremental ----------------------------
  // chest 对应 PoseConstraintList[POSE_DATA_LIST_INDEX_CHEST]，用于 waist_yaw_link 的位置代价约束
  Eigen::Vector3d robotChestPosAnchor_ = Eigen::Vector3d::Zero();
  Eigen::Vector3d humanChestPosAnchor_ = Eigen::Vector3d::Zero();
  Eigen::Vector3d robotChestDeltaPos_ = Eigen::Vector3d::Zero();  // δ
  Eigen::Vector3d dotChestDeltaPos_ = Eigen::Vector3d::Zero();    // dδ/dt
  // ---------------------------- chest position incremental ----------------------------
  ...
  void saveLastOnExit(const std::vector<PoseData>& latestPoseConstraintList) {
    if (latestPoseConstraintList.size() > POSE_DATA_LIST_INDEX_CHEST) {
      robotChestPosAnchor_ = latestPoseConstraintList[POSE_DATA_LIST_INDEX_CHEST].position;
      robotChestDeltaPos_.setZero();
      dotChestDeltaPos_.setZero();
    }
 ...
 public:
  ...
  // chest position getters
  Eigen::Vector3d getLatestRobotChestPos() const { return robotChestPosAnchor_ + robotChestDeltaPos_; }
  Eigen::Vector3d getRobotChestAnchorPos() const { return robotChestPosAnchor_; }
  Eigen::Vector3d getHumanChestAnchorPos() const { return humanChestPosAnchor_; }
  ...
  friend class WheelIncrementalControlModule;
};
...
class WheelIncrementalControlModule {
 public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  explicit WheelIncrementalControlModule(const IncrementalControlConfig& config = IncrementalControlConfig{});
  ~WheelIncrementalControlModule();

  WheelIncrementalControlModule(const WheelIncrementalControlModule&) = delete;
  WheelIncrementalControlModule& operator=(const WheelIncrementalControlModule&) = delete;

  // 左右手独立进入增量模式接口
  // chest：任意手进入增量模式时触发一次，第二只手进入时不重复触发
  void enterIncrementalModeChest(const Eigen::Vector3d& humanChestPos,
  ...
  Eigen::Vector3d getLatestRobotChestPos() const;
  ...
  void updateRightArmPoseAnchor(const ArmPose& vrRightPose,
                                ...
                                const Eigen::Quaterniond& qEndEffector = Eigen::Quaterniond::Identity(),
                                const Eigen::Quaterniond& qLink4 = Eigen::Quaterniond::Identity());
  void updateChestAnchorOnExit(const Eigen::Vector3d& humanChestPos,
                               const std::vector<PoseData>& latestPoseConstraintList);
  ...
  bool isIncrementalModeChest() const;
 ...
 private:
  ...
  bool chestActivated_ = false;  // chest 增量是否激活（随任意手进入激活，双手都退出才失活）
