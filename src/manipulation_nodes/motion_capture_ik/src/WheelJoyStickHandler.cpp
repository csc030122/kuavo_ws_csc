...
WheelJoyStickHandler::WheelJoyStickHandler(double threshold, double alpha)
    : isInitialized_(false),
      endEffectorType_(EndEffectorType::LEJUCLAW),
      joyStickThreshold_(threshold),
      joyStickAlpha_(alpha) {}

void WheelJoyStickHandler::initialize() {
  if (isInitialized_.load()) {
    std::cout << "\033[93m[WheelJoyStickHandler] Already initialized, skipping\033[0m" << std::endl;
    return;
  }
  ...
  std::cout << "\033[92m[WheelJoyStickHandler] Initialization completed\033[0m" << std::endl;
}

void WheelJoyStickHandler::reset() {
...
double WheelJoyStickHandler::getLeftJoyStickX() const {
  std::lock_guard<std::mutex> lock(dataMutex_);
  // print leftJoyStickX_
  // std::cout << "\033[92m[WheelJoyStickHandler] leftJoyStickX_: " << leftJoyStickX_ << "\033[0m" << std::endl;
  return leftJoyStickX_;
}

double WheelJoyStickHandler::getLeftJoyStickY() const {
  std::lock_guard<std::mutex> lock(dataMutex_);
  // std::cout << "\033[92m[WheelJoyStickHandler] leftJoyStickY_: " << leftJoyStickY_ << "\033[0m" << std::endl;
  return leftJoyStickY_;
}

double WheelJoyStickHandler::getRightJoyStickX() const {
  std::lock_guard<std::mutex> lock(dataMutex_);
  // std::cout << "\033[92m[WheelJoyStickHandler] rightJoyStickX_: " << rightJoyStickX_ << "\033[0m" << std::endl;
  return rightJoyStickX_;
}

double WheelJoyStickHandler::getRightJoyStickY() const {
  std::lock_guard<std::mutex> lock(dataMutex_);
  // std::cout << "\033[92m[WheelJoyStickHandler] rightJoyStickY_: " << rightJoyStickY_ << "\033[0m" << std::endl;
  return rightJoyStickY_;
}

bool WheelJoyStickHandler::getRightJoyStickYHold() const {
  std::lock_guard<std::mutex> lock(dataMutex_);
...
void WheelJoyStickHandler::updateJoyStickData(const noitom_hi5_hand_udp_python::JoySticks::ConstPtr& msg) {
  std::lock_guard<std::mutex> lock(dataMutex_);
  if (!isInitialized_.load()) {
    std::cout << "\033[91m[WheelJoyStickHandler] Not initialized, call initialize() first\033[0m" << std::endl;
    return;
  }
  ...
  if (leftFirstButtonPressed_ && !leftButtonXLastPressed_) {
    ...
    if (!leftButtonXFirstPressTime_.isZero() && now < leftButtonXFirstPressTime_) {
      ...
      if (anyHandRecentlyChanged) {
        // 任意手最近有变化，阻止左手模式切换
        std::cout << "\033[93m[WheelJoyStickHandler] 左手控制模式切换被阻止：在 "
  ...
  if (rightFirstButtonPressed_ && !rightButtonALastPressed_) {
    ...
    if (!rightButtonAFirstPressTime_.isZero() && now < rightButtonAFirstPressTime_) {
      ...
      if (anyHandRecentlyChanged) {
        // 任意手最近有变化，阻止右手模式切换
        std::cout << "\033[93m[WheelJoyStickHandler] 右手控制模式切换被阻止：在 "
                  << (now - lastArmCtrlModeChangeTime_).toSec() << " 秒前发生了模式变化\033[0m" << std::endl;
        // 重置双击检测状态
...
void WheelJoyStickHandler::processHandEndEffectorData() {
  std::lock_guard<std::mutex> lock(dataMutex_);
  if (!isInitialized_.load()) {
    std::cout << "\033[91m[WheelJoyStickHandler] Not initialized, call initialize() first\033[0m" << std::endl;
    return;
...
void WheelJoyStickHandler::processHandEndEffectorDataWithFingerTracking() {
  std::lock_guard<std::mutex> lock(dataMutex_);
  if (!isInitialized_.load()) {
    std::cout << "\033[91m[WheelJoyStickHandler] Not initialized, call initialize() first\033[0m" << std::endl;
    return;
...
void WheelJoyStickHandler::loadHandControlParameters() {
  try {
    ros::NodeHandle nh;
    ...
    ROS_INFO("[WheelJoyStickHandler] End effector type: %s", endEffectorTypeStr.c_str());
  } catch (const std::exception& e) {
    std::cout << "\033[91m[WheelJoyStickHandler] Error reading ROS parameters: " << e.what()
              << ", using default qiangnao\033[0m" << std::endl;
    endEffectorType_ = EndEffectorType::QIANGNAO;
  }
}

void WheelJoyStickHandler::processRobotEndHandWithFingerData() {
  if (!isInitialized_.load()) {
    std::cout << "\033[91m[WheelJoyStickHandler] Not initialized, call initialize() first\033[0m" << std::endl;
    return;
  }
...
HandPositionData WheelJoyStickHandler::getHandPositionData() {
  std::lock_guard<std::mutex> lock(dataMutex_);
  if (!isInitialized_.load()) {
    std::cout << "\033[91m[WheelJoyStickHandler] Not initialized, call initialize() first\033[0m" << std::endl;
...
ClawCommandData WheelJoyStickHandler::getClawCommandData() {
  std::lock_guard<std::mutex> lock(dataMutex_);
  if (!isInitialized_.load()) {
    std::cout << "\033[91m[WheelJoyStickHandler] Not initialized, call initialize() first\033[0m" << std::endl;
...
EndEffectorType WheelJoyStickHandler::getEndEffectorType() const { return endEffectorType_.load(); }

double WheelJoyStickHandler::limitValue(double value, double minVal, double maxVal) {
  return std::max(minVal, std::min(maxVal, value));
}

int WheelJoyStickHandler::limitIntValue(int value, int minVal, int maxVal) const {
  return std::max(minVal, std::min(value, maxVal));
}

void WheelJoyStickHandler::processHandFingerDataWithJoystick() {
  // ROS_INFO("[WheelJoyStickHandler] Processing hand finger data with joystick");

  if (freezeFinger_) {
    // 冻结模式：使用冻结的值
    // ROS_INFO("[WheelJoyStickHandler] Finger is frozen, using frozen values");
...
void WheelJoyStickHandler::processHandFingerDataWithHandTracking() {
  ROS_INFO("[WheelJoyStickHandler] Processing hand finger data with hand tracking");
...
void WheelJoyStickHandler::processClawDataWithJoystick() {
  if (freezeFinger_) {
    // 冻结模式：使用冻结的值
...
void WheelJoyStickHandler::processClawDataWithHandTracking() {
  if (freezeFinger_) {
    // 冻结模式：使用冻结的值
    clawPosition_ = frozenClawPosition_;
  } else {
...
bool WheelJoyStickHandler::hasLeftArmCtrlModeChanged() {
  std::lock_guard<std::mutex> lock(dataMutex_);
  // 比较当前左手状态和上次查询状态
...
bool WheelJoyStickHandler::hasRightArmCtrlModeChanged() {
  std::lock_guard<std::mutex> lock(dataMutex_);
  // 比较当前右手状态和上次查询状态
...
bool WheelJoyStickHandler::isLeftFirstButtonPressed() const {
  std::lock_guard<std::mutex> lock(dataMutex_);
  return leftFirstButtonPressed_;
}

bool WheelJoyStickHandler::isRightFirstButtonPressed() const {
  std::lock_guard<std::mutex> lock(dataMutex_);
  return rightFirstButtonPressed_;
}

bool WheelJoyStickHandler::isLeftSecondButtonPressed() const {
  std::lock_guard<std::mutex> lock(dataMutex_);
...
bool WheelJoyStickHandler::isLeftRightFirstButtonPressed() const {
  std::lock_guard<std::mutex> lock(dataMutex_);
  return leftFirstButtonPressed_ && rightFirstButtonPressed_;
}

bool WheelJoyStickHandler::isLeftGrip() const {
  std::lock_guard<std::mutex> lock(dataMutex_);
  return leftGrip_;
}

bool WheelJoyStickHandler::isRightGrip() const {
  std::lock_guard<std::mutex> lock(dataMutex_);
  return rightGrip_;
}

bool WheelJoyStickHandler::isLeftRightGrip() const {
  std::lock_guard<std::mutex> lock(dataMutex_);
  return leftGrip_ && rightGrip_;
}

bool WheelJoyStickHandler::isLeftArmCtrlModeActive() const {
  std::lock_guard<std::mutex> lock(dataMutex_);
  return leftArmCtrlModeActive_;
}

bool WheelJoyStickHandler::isRightArmCtrlModeActive() const {
  std::lock_guard<std::mutex> lock(dataMutex_);
  return rightArmCtrlModeActive_;
}

void WheelJoyStickHandler::forceSetLeftArmCtrlMode(bool active) {
  std::lock_guard<std::mutex> lock(dataMutex_);
  ...
  if (leftArmCtrlModeActive_ != active) {
    ...
    // std::cout << "\033[92m[WheelJoyStickHandler] 强制设置左手控制模式: " << (active ? "激活" : "停用")
    //           << "，已设置5秒超时保护\033[0m" << std::endl;
  }
}

void WheelJoyStickHandler::forceSetRightArmCtrlMode(bool active) {
  std::lock_guard<std::mutex> lock(dataMutex_);
  ...
  if (rightArmCtrlModeActive_ != active) {
    ...
    // 更新模式变化时间戳，设置5秒超时保护，防止其他按键切换
    lastArmCtrlModeChangeTime_ = ros::Time::now();
    // std::cout << "\033[92m[WheelJoyStickHandler] 强制设置右手控制模式: " << (active ? "激活" : "停用")
    //           << "，已设置5秒超时保护\033[0m" << std::endl;
  }
}
